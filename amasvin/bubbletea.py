...
class Bubbletea(Drink):
    ...
    def __init__(self, name, price):
        super().__init__(name, price)   # 매우 중요!! # 부모의 생성자 호출하자
        self.pearl = 0 # 0: 타피오카, 1: 코코, 2: 알로에, 3: 곤약

    def __str__(self):
        return super().__str__() \
               + f'\t펄: {Bubbletea._PEARLS[self.pearl]}'

    def set_pearl(self):
        # 펄 종류 출력 1. ~, 2. ~
        for index, bubble_label in enumerate(Bubbletea._PEARLS):
            print(f'{index + 1}. {bubble_label}')
        # 사용자에게 입력받자
        pearl = input('펄을 선택하세요: ')
        # 그냥 엔터면 기본값, 숫자 입력받은 것은 index로 변경
        # if pearl == '':
        #     self.pearl = 0
        # else:
